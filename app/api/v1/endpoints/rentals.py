...
def start_rental(payload: RentalCreate, db: Session = Depends(get_db)) -> RentalResponse:
    try:
    ...
    logger.info(
        ...
        rental.id, rental.user_id, rental.car_id,
    )
...
def return_rental(rental_id: int, db: Session = Depends(get_db)) -> RentalResponse:
    ...
    logger.info(
        ...
        rental.id, rental.car_id, rental.end_time.isoformat(),
    )
    return RentalResponse.model_validate(rental)
